 ...
 *
 * **業種で絞れるのは掲示板の上位だけ。** バナーは全員に同じものを出す。
 * 「10枠が3秒ずつ」で全員に届くのがバナーの値打ちなので、ここを業種で
 * 絞ると、同じ金額で届く人数だけが減る。どちらに向くかは `fit` に書いた。
 */
export const adPlacements = [
  {
    ...
    /** 業種では絞り込めない枠。会員全員に同じものが出る。 */
    fit: '会員全体へ広く知っていただきたい方',
    slots: 10,
  },
  {
    key: 'list',
    ...
    /** **業種を指定できるのはこちらだけ。** 絞れば届く人数は減るが、確度は上がる。 */
    fit: '特定の業種の方へ確実に届けたい方',
    slots: 3,
  },
] as const;
 ...
 * 1回の申し込みで買える最短の日数。
 *
 * **1日は受けない。** 1日だけの予約が乱立すると、カレンダーが虫食いになって
 * 空きが読めなくなる。2日あれば、飛び石で埋まっても隣と繋げて見られる。
 *
 * もとは7日だった。試しに1回出してみたい人に「まず7日ぶん」を求めるのは
 * 重く、当たった無料券（1日分・3日分）も7日ためるまで使えなかった。
 */
export const AD_MIN_DAYS = 2;

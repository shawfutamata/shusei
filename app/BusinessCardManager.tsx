...
function ReviewCard({ draft, preview, index, total, onChange, onPrevious, onNext, onConfirm }: {
  ...
  return <div className="cardbook-body review-view"><div className="review-progress"><b>{index + 1} / {total}枚目</b><span>{Array.from({ length: total }).map((_, item) => <i className={item === index ? 'active' : ''} key={item} />)}</span></div><img className="review-image" src={preview} alt={`${index + 1}枚目の名刺`} /><p className="review-guide">名刺に記載がない項目は空欄で登録できます。読み取り間違いは直接修正してください。</p><CardFields card={draft} onChange={onChange} /><div className="review-nav"><button onClick={onPrevious} disabled={index === 0}>‹ 前の名刺</button>{index < total - 1 ? <button className="next" onClick={onNext}>次の名刺 ›</button> : <button className="next" onClick={onConfirm}>登録内容を確認 ›</button>}</div></div>;
}

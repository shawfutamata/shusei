...
const sections = [
  ['1. 取得する情報', '本サービスは、メールアドレス、氏名・顔写真・会社名・所属会場・役職・バッヂ・活動エリア・業種・任意の年商区分、投稿・紹介内容、探しごとに添えた写真、広告枠に入稿された文言・リンク・画像、端末の通知トークン、利用日時やエラー等の技術情報を取得します。'],
  ['2. 利用目的', '会員本人の確認、プロフィール表示、探しごとと紹介の提供、関連業種の通知、広告枠の掲載と請求、不正利用防止、問い合わせ対応、品質・安全性の改善のために利用します。'],
  ...
  ['6. 保存期間と削除', '利用目的に必要な期間だけ情報を保存します。アプリの「マイページ」からアカウントを削除すると、プロフィール、投稿、紹介、登録された画像および認証セッションを削除します。法令上の保存義務または不正対策上必要な記録は、必要期間に限り保持する場合があります。'],

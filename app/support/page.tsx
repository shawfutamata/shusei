...
const faqs = [
  ...
  ['案件に写真を添えたい', '投稿の画面から写真を1枚まで選べます。写真は端末の中で小さく作り直してから送るので、通信量を抑えられます。一覧では正方形に切り抜いて表示されます。'],
  ['おすすめに案件が出ません', 'マイページの「おすすめに出したい業種」が設定されているかご確認ください。設定した業種の案件が投稿されると、ホームの「あなたにおすすめの案件」に並びます。'],
  ['仲間を招待したい', 'マイページの「仲間を招待する」から、あなた専用の招待リンクをコピーして共有してください。リンクから登録された方は、そのままご利用いただけます。'],
  // 招待の無料月は止めてある（`db/data.ts` の REFERRAL_FREE_MONTHS）。戻すときは、
  // この答えを「招待した方のご利用が7日続くと、有料プランの方は会費が1ヶ月分安くなり、
  // 無料プランの方はスタンダードが1ヶ月使えるようになります（合計6ヶ月分まで）」に戻す。
  ['招待の特典について', '招待リンク（または招待コード）から入会された方の人数で、あなたのランクが上がります。ランクが上がると、案件に付けられる写真や動画、掲示板の絞り込みなど、使えるものが増えます。お手続きは要りません。ランクの特典は、予告なく内容の変更または終了をすることがあります。'],
  ['顔写真の登録は必須ですか', '必須です。誰からの案件・オファーなのかが分かることが、安心してオファーし合うための前提だからです。'],

...
import { planCatalog, planPrice, yearlyYen } from '../plan-catalog';
...
export default function TokushohoPage() {
  ...
  const rows: [string, React.ReactNode][] = [
    ...
    ['販売価格', <>
      ...
      プレミアム：{planPrice('premium')}（税込）／{planPrice('premium', 'year')}（税込）<br />
      無料プランは料金がかかりません。
    </>],
    ...
    ['支払時期', <>
      ...
      以降は、月払いは毎月、年払いは毎年、お申し込み日に対応する日に自動的に更新し、同額をご請求します。
    </>],
  ...
  return <LegalPage
    ...
    title="特定商取引法に基づく表記"
    lead={`${serviceName}の有料プランに関する販売条件です。`}
  ...
  >
    ...
    <LegalSection
      heading="価格の内訳"
      body={[
        `年払いは、月払いの12か月ぶんから20%を割り引いた金額です。スタンダードは月あたり ${Math.round(yearlyYen('standard') / 12).toLocaleString('ja-JP')}円、プレミアムは月あたり ${Math.round(yearlyYen('premium') / 12).toLocaleString('ja-JP')}円になります。`,

...
const statements = [
  ...
  // 日付は日本時間の YYYY-MM-DD。
  `CREATE TABLE IF NOT EXISTS gacha_days (
    member_id TEXT NOT NULL REFERENCES members(id),
    day TEXT NOT NULL,
    -- どの回か（app/gacha.ts の key）。やり直すときは新しい名前にする。
    campaign_key TEXT NOT NULL,
    -- クリスマスかお正月か。あとから内訳を見るために残す。
  ...
  // 1人1回だったころの表。**消さない。** 消すと、この名前で残っている
  // 古い記録が読めなくなる。いまは使っていない。
...
export async function getMembershipAccess(userId: string): Promise<MembershipAccess> {
  ...
  const row = await env.DB.prepare(`SELECT membership_status AS status, membership_source AS source,
    membership_period_end AS currentPeriodEnd, organization_id AS organizationId, email
    FROM members WHERE id = ?`).bind(userId).first<Omit<MembershipAccess, 'canUseApp'> & { email: string }>();
 ...
 * 新しいメッセージを見落とさないようにする。
 *
 * 止めたい人は、届いたメールに返信してもらう（`mail_on_message` を運営が0にする）。
 * **会員が自分で切る画面はまだ無い。** メールの文面にもそう書いてある。
 * 通数がそのまま送信の費用になるので、増えてきたら切る道を先に用意すること。
 *
...
export async function getMailOnMessage(memberId: string) {
  await ensureDatabase();
  const row = await env.DB.prepare('SELECT mail_on_message AS mailOn FROM members WHERE id = ?')
    .bind(memberId).first<{ mailOn: number }>();
  return Number(row?.mailOn ?? 1) === 1;
}

/** メールのお知らせを切り替える。会員が自分で止められるようにしておく。 */
export async function setMailOnMessage(memberId: string, on: boolean) {
  await ensureDatabase();
...
export type AdSlot = {
   ...
   *
   * **効き方が枠によって違う。**
   * - `list`（掲示板の上位）… 絞り込み。その業種の会員にだけ出る。
   * - `banner`（画面上部）… 並べ替えだけ。**誰に出るかは変えない**。その業種の
   *   会員の画面で先のほうへ回るだけで、全員に出るという約束は変わらない。
   *
   * 当てる相手は**会員が登録している業種**（`primary_industry` と
   * `notify_industries`）で決める。画面でタップしている絞り込みではない。
   * 絞り込みはほとんどの人が触らないので、そちらで当てると出稿した広告が
   * ほぼ誰にも出なくなる（実際そうなっていた）。
   */
  industry: string;
  memberName: string;
...
export async function getMemberRank(memberId: string) {
  ...
  const row = await env.DB.prepare(`SELECT m.email,
    (SELECT COUNT(*) FROM members inv WHERE inv.invited_by = m.id) AS inviteCount
 ...
 * 「引いたか」を先に SELECT で調べてから INSERT すると、その隙間に2回目が入る。
 */
export async function drawGacha(memberId: string): Promise<GachaState> {
  await ensureDatabase();
  const season = gachaSeason();
  if (!season) return getGachaState(memberId);
  ...
  // 当たったが、上限までの残りで足りない。**減らして渡さない。**
  // 「3日分」と出しておいて1日しか入らないほうが、はずれよりも裏切りになる。
  if (prize.days > left) prize = consolationPrize(season);

  const now = new Date().toISOString();
  await env.DB.prepare(`INSERT OR IGNORE INTO gacha_days (member_id, day, campaign_key, season, prize_key, days, drawn_at)
    VALUES (?, ?, ?, ?, ?, ?, ?)`)
    .bind(memberId, today, adGacha.key, season.key, prize.key, prize.days, now).run();

  // 入ったのが自分の行かどうかを読み直して確かめる。負けていたら券は出さない。
  const saved = await env.DB.prepare('SELECT prize_key AS prizeKey FROM gacha_days WHERE member_id = ? AND day = ?')
    .bind(memberId, today).first<{ prizeKey: string }>();

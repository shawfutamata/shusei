...
- 法人（組織）アカウントとして登録できるなら、この要件は課されない。`jp.everycounts` の名義で法人登録できるかを先に確認する
- 法人登録にはD-U-N-S番号などの確認が要る場合がある。取得に日数がかかる
- 要件は変更されるので、**Play Console の最新の案内を必ず確認する**

→ 判断が必要: 法人アカウントで登録するか、個人アカウントでテスト期間を織り込むか。

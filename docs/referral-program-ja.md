...
> **いまは無料月を配っていない。** 2026年12月31日まではキャンペーンで全員がスタンダードを
> 無料で使えるので、そのうえに無料月を重ねても招待する側に何も増えない。
> 切っているのは `db/data.ts` の `REFERRAL_FREE_MONTHS = false` の1行だけで、
> 判定も付与もそこで止まる。`true` に戻せば、この文書のとおりにまた動く。
>
> **止めていないもの**: 招待リンク・招待コード・`invited_by` の記録・
> 招待人数によるランク（`docs/rank-perks-ja.md`）。招待そのものは今までどおり集める。
>
> **止めているあいだの扱い**: 新しいクレジットは作らない。すでに `referral_credits` に
> ある行（`earned` を含む）も請求には当てない（`applyReferralCreditsToStripe` が 0 を返す）。
> 行は消していないので、戻したときにそのまま拾える。
...
対象が経営者・事業主に限られた**閉じた市場**なので、「◯人紹介したら永久無料」は採用しない。永久無料にすると、会員が増えきった時点で全員が無料の資格を持ち、売上がゼロに向かうため。
...
**承知のうえで緩くしてある。** 誰でも登録できるので、無料会員を6人連れてくれば紹介者は半額になり、その6人が無料のままなら売上は増えない。原理的には赤字が出る。

それでもこの条件で始めるのは、**空の掲示板に月5,000円を払う人はいない**から。最初の数十人は割引を配ってでも入れて、投稿が並んでいる状態を作るほうが先。

### 切り替えの目安

**有料会員が100人を超えたら**、資格条件を見直す。候補は2つ。

| | 変更 | 効果 |
|---|---|---|
| A | 招待された人が**有料になったら**確定 | 赤字が原理的に出ない。紹介のハードルは上がる |
| B | `REFERRAL_CAP_TOTAL` を 6 → 3 | 上限が半分になる。1行で終わる |

いつでも変えられるように、**会員向けの画面と `docs` の両方に「予告なく変更・終了することがある」と明記してある**（マイページの招待カード末尾と、サポートページのFAQ「招待の特典について」）。**すでに確定したぶんは取り消さない**と書いてあるので、変更するときもそこは守ること。
...
| 無料会員 | **スタンダードが1ヶ月使える** | その場で `bonus_plan='standard'`、`bonus_period_end` を1ヶ月延長、`applied_month` を記録 |

### 年払いの人がもらう額を「割引後の月あたり」にしている理由

年払いは20%OFFなので、スタンダードなら年11,520円＝月あたり960円。ここで定価の1,200円を返すと、**20%OFFと紹介の無料月が二重取り**になり、6人紹介すると年11,520円のうち7,200円が引かれて実質62%OFFになってしまう。

割引後の月あたり額（960円）にすれば、6人でも5,760円＝**ちょうど半額**で、月払いの人と同じ「通算で最大でも半額」に収まる。1つの上限で両方の周期を守れる。

金額の計算は `monthlyEquivalentYen(plan, cycle)`（`app/plan-catalog.ts`）1箇所にある。

### いつ、どこで確定するか

**運営の手作業は要らない。** 資格判定と付与は `syncReferralBenefits()` が全部やる。呼ばれるのは次の3か所で、どれか1つでも通れば確定する。

| 経路 | 誰が通るか |
|---|---|
| `GET /api/entitlements` | アプリの起動時、Webが「いま何が使えるか」を聞くとき。**招待画面を開かない会員もここで拾える** |
| `GET /api/referral` | Webのマイページ（招待カード） |
| `GET /api/invite` | アプリの招待画面 |

契約していない会員には、その場で `bonus_plan='standard'` が入る。契約している会員には、**確定した時点でStripeの顧客残高に入れる**（`app/billing-credits.ts`）。残高は次回以降の請求から自動で引かれるので、消し込みも要らない。
...
earned_at      TEXT  -- 確定した日。確定していないあいだは空

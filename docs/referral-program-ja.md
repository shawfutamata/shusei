...
守成クラブは会場約450、会員数に上限がある**閉じた市場**なので、「◯人紹介したら永久無料」は採用しない。永久無料にすると、会員が増えきった時点で全員が無料の資格を持ち、売上がゼロに向かうため。
...
3. 招待された人が `active` になって **60日** 続いたら、紹介した人に**無料1ヶ月**が確定する。
4. 上限は**直近12ヶ月で6ヶ月ぶん**。7人目以降は `capped` として記録され、無料月にはならない（会員ランクに反映する方針）。
5. 60日に届く前に解約した人は、無料月にならない。
...
ベースは無料サービスなので、確定した1ヶ月ぶんのクレジットは、紹介した人のプランで意味が変わる。**仕組みは1つのまま。**
...
契約が済んだら有効にする。`activated_at` を入れておくと、60日の起算日がはっきりする（省略するとシステムが気づいた日を入れる）。
...
会員がマイページを開いたときに、資格判定と確定が自動で走る（何度走らせても結果は同じ）。請求のときは、まだ使っていない無料月を見る。
...
activated_at  TEXT  -- activeになった日。60日の起算点

-- referral_credits（招待された人1人につき最大1行）
inviter_id     TEXT
invitee_id     TEXT UNIQUE
status         TEXT  -- earned（無料1ヶ月）| capped（年の上限超え）
earned_at      TEXT

...
export default function ProfileScreen() {
  ...
  return <AppScreen title="マイページ" eyebrow="MY PAGE">
    ...
    {!!stats && <View style={commonStyles.card}>
      ...
      <View style={styles.perkGrid}>{rankPerks.map((perk) => {
        ...
        return <Pressable key={perk.key} style={styles.perkTile} onPress={() => Alert.alert(perk.label, perk.detail + (perk.soon ? '\n\nこの特典はまだ作っている途中です。' : unlocked ? '' : `\n\nあと${Math.max(0, rankThresholds[perk.minLevel - 1] - stats.inviteCount)}人の仲間をご招待いただくと使えるようになります。`))}>
          ...
          {perk.soon || !unlocked
            ? <Text style={[styles.perkChip, perk.soon && styles.perkChipSoon]}>{perk.soon ? '近日公開' : rankNames[perk.minLevel - 1]}</Text>
            : <View style={styles.perkChipSpacer} />}
          <Text style={[styles.perkLabel, !unlocked && styles.perkLabelLocked]} numberOfLines={2}>{perk.label}</Text>
        </Pressable>;
      })}</View>
    </View>}
    ...
    <View style={styles.contractNote}><Ionicons name="shield-checkmark-outline" size={25} color={AppColors.blue} /><View style={{ flex: 1 }}><Text style={styles.contractTitle}>会員専用</Text><Text style={styles.contractText}>登録済みの会員情報と利用状態を安全に確認しています。</Text></View></View>
...
const styles = StyleSheet.create({
  ...
  perkNow: { color: AppColors.accentInk, fontSize: 12, fontWeight: '900' },
  perkLead: { marginTop: 6, color: AppColors.muted, fontSize: 11, lineHeight: 18, fontWeight: '700' },
  perkGrid: { marginTop: 14, flexDirection: 'row', flexWrap: 'wrap', rowGap: 14 },
  perkTile: { width: '25%', alignItems: 'center', gap: 5 },
  perkBadge: { width: 52, height: 52, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: '#CFE0FB', borderRadius: 26, backgroundColor: '#EAF3FF' },
  perkBadgeLocked: { borderColor: '#CCD5E1', borderStyle: 'dashed', backgroundColor: '#F4F6F9' },
  perkChip: { height: 14, paddingHorizontal: 6, borderRadius: 99, overflow: 'hidden', backgroundColor: '#59677D', color: '#fff', fontSize: 8, lineHeight: 14, fontWeight: '900' },
  perkChipSoon: { backgroundColor: AppColors.accentInk },
  perkChipSpacer: { height: 14 },
  perkLabel: { paddingHorizontal: 2, color: '#3A4A63', fontSize: 10, lineHeight: 14, fontWeight: '800', textAlign: 'center' },
  perkLabelLocked: { color: '#9AA6B8' },

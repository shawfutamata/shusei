...
export default function CreateScreen() {
  const [category, setCategory] = useState('案件'); const [title, setTitle] = useState(''); const [description, setDescription] = useState(''); const [budget, setBudget] = useState(''); const [budgetBand, setBudgetBand] = useState(''); const [area, setArea] = useState(''); const [industry, setIndustry] = useState(''); const [busy, setBusy] = useState(false);
  async function submit() {
    if (!title.trim() || !description.trim() || !budgetBand || !area.trim() || !industry) return Alert.alert('入力を確認してください', 'すべての項目を入力してください。');
    setBusy(true);
    try {
      await apiFetch('/api/board', { method: 'POST', body: JSON.stringify({ category: category === '案件' ? 'project' : category === '協業先' ? 'collaboration' : 'consultation', title, description, budgetLabel: budget, budgetBand, area, industryTags: [industry], deadline: new Date(Date.now() + 30 * 86400_000).toISOString().slice(0, 10) }) });

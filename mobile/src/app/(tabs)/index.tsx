...
export default function HomeScreen() {
  ...
  const [ads, setAds] = useState<{ ad: AdSlot; source: ApiImageSource }[]>([]);
  ...
  useFocusEffect(useCallback(() => {
    ...
    Promise.all([apiFetch<{ requests: RequestItem[]; ads?: AdSlot[] }>('/api/board'), getRequestPreferences(user.userId)]).then(async ([board, preferences]) => {
      ...
      const shown = (board.ads ?? []).filter((ad) => ad.imageUrl);
      const sources = await Promise.all(shown.map(async (ad) => ({ ad, source: await apiImageSource(ad.imageUrl) })));
      if (!active) return;
      setAds(sources.filter((entry): entry is { ad: AdSlot; source: ApiImageSource } => Boolean(entry.source)));
    }).catch(() => undefined);
    return () => { active = false; };
  }, [user]));
